...
#pragma mark ---Текстовое поле из одной картинки без возможности вставить текст ---
...
+ (CBTNoPasteTextField *)noPasteTextFieldWithImageNamed:(NSString *)imageName margin:(float)leftMargin fontName:(NSString *)fontName fontSize:(float)fontSize fontColor:(NSString *)fontColor andPlaceholder:(NSString *)placeholder{
    CBTNoPasteTextField *txtField = [[CBTNoPasteTextField alloc] init];
    [txtField setBorderStyle:UITextBorderStyleNone];
...
#pragma mark ---Текстовое поле из трёх картинок в которое нельзя вставить текст---
...
+ (NSString *) apiHostName{
    return @"http://yash.sg.umbrella-web.com/api/";
}
